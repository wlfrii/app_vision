...
uint8_t dm_util::hammingDistance(uint32_t x, uint32_t y)
{
...
#define preAggregate(gray_prev, cost_array_prev, min_cost_prev)             \
    memcpy(cost_aggr_path, cost_init_path, d_range * sizeof(uint8_t));      \
    /* The previous cost array in the path, two extra elements assigned for
       avoiding out of range in head/tail when aggregation. */              \
    std::vector<uint8_t> cost_array_prev(d_range + 2, UINT8_MAX);           \
    /* Retrieve the initial cost of previous pixel */                       \
    memcpy(&cost_array_prev[1], cost_aggr_path, d_range * sizeof(uint8_t)); \
    /* Caculate the minimum path cost of the the first pixel
       and regard it as the priveous for the later pixels */                \
    uint8_t min_cost_prev = UINT8_MAX;                                      \
...
void doAggregate(int P1, int P2, uint8_t gray_curr, uint8_t& gray_prev,
               uint16_t d_range, uint8_t& min_cost_prev, const uint8_t* cost_init_path,
               uint8_t* cost_aggr_path, std::vector<uint8_t>& cost_array_prev)
{
    ...
    uint8_t min_cost_curr = UINT8_MAX;
    for (uint16_t d = 0; d < d_range; d++)
    {
         ...
         *              - min(Lr(p-r,k))
         */
        uint8_t cost = cost_init_path[d];
        // Note, it's possible to get a Lr_x greater than UINT8_MAX
        uint16_t Lr_1 = cost_array_prev[d + 1];
        uint16_t Lr_2 = cost_array_prev[d] + P1;
        uint16_t Lr_3 = cost_array_prev[d + 2] + P1;
        uint16_t Lr_4 = min_cost_prev + P2_real;

        uint8_t Lr = cost + std::min(std::min(Lr_1, Lr_2), std::min(Lr_3, Lr_4)) - min_cost_prev;
        cost_aggr_path[d] = Lr;

...
bool SGM::initialize(uint16_t width, uint16_t height, const Option& option)
{
    ...
    _cost_init  = new uint8_t[_cost_size]();
    _cost_aggr  = new uint16_t[_cost_size]();
    for(int i = 0; i < 8; i++){
        _cost_aggr_path[i] = new uint8_t[_cost_size]();
    }
...
{
    dm_util::censusTransform(_l_image, _l_census_values, _image_width, _image_height);
    dm_util::censusTransform(_r_image, _r_census_values, _image_width, _image_height);
    ...
    for (uint16_t v = 0; v < _image_height; v++)
    {
        for (uint16_t u = 0; u < _image_width; u++)
        {
            ...
            for(uint16_t d = d_min; d < d_max; d++)
            {
                ...
                // For right disparity, based on, R_cost(xr,yr,d) = L_cost(xr+d,yl,d)
                if(!is_left){
                    if(u < _image_width - d){

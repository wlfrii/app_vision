...
{
    closeDevice();
    DELETE_ARRAY_PIONTER(decode_buffer);
    DELETE_ARRAY_PIONTER(jpeg_buffer);
}
...
bool V4L2Capture::ioctlDequeueBuffers(cv::Mat &out)
{
    std::lock_guard<std::mutex> lck(mtx);
    if(cameraFd < 0)
        return false;
    ...
    if(vbuffer.length > 0)
    {
        ...
        decompress_mjpeg_success = processImage(buffer_mmap_ptr[vbuffer.index].addr, vbuffer.length, out);
...
bool V4L2Capture::processImage(const void *p, uint size, cv::Mat &out)
{
    ...
    if(bSuccess)
    {
        out = cv::Mat(frame_height, frame_width, CV_8UC3, decode_buffer);
    }
...
void V4L2Capture::resetDevice()
{
    if(cameraFd != -1)
    {
        ...
        unMmapBuffers();
        DELETE_ARRAY_PIONTER(buffer_mmap_ptr);
        closeDevice();
        CLEAR(device_name);
    }
}
